...
public class TeacherDAOImpl extends User implements TeacherDAO {
    public TeacherDAOImpl(String username, String password, String name) {
...
//    教师：填写/维护教材订购表（教务系统上有下载可参考），提交后不能删除
    ...
    public OrderForm updateOrderForm(OrderForm orderForm, Integer integer) throws Exception {
        ...
        QueryRunner queryRunner = new QueryRunner();
        // 更新订购表
        String sql = "update order_forms set department = ? , course = ? , textbook_name = ? ," +
                " publisher = ? , quantity = ?  , is_approved_by_department_head = ? where id = ?";
        int id = integer;
        int affectedRow = queryRunner.update(connection, sql, orderForm.getDepartment(),
                orderForm.getCourse(), orderForm.getTextbook_name(),
